...
export default function LoginPage() {
  ...
  const { timer: otpTimer, start: startTimer, reset: resetTimer } = useOtpTimer(60);

  const fullPhone = `+91${phone}`;

  // ── Derive the OTP purpose from the current mode ─────────────────────────
  // This is the key alignment between frontend and backend:
  //   "login" mode    → purpose: "login"
  //   "register" mode → purpose: "registration"
  ...
  const handleRequestOtp = async () => {
    ...
    try {
      const { data } = await authApi.requestOtp({
        phone: fullPhone,
        purpose: otpPurpose,  // ← ALIGNED with the current mode
        channel: "sms",
      });
 ...
 console.log(payload);
    ...
    try {
      ...
      if (mode === "register") {
        // ── REGISTRATION FLOW ───────────────────────────────────────────────
        // Pure registration: just call register, no fallback.
        // If the user is already registered, the backend returns 409 Conflict.
        response = await authApi.registerPassenger(payload);

      } else {
        // ── LOGIN FLOW ──────────────────────────────────────────────────────
        // Pure login: call loginWithOtp. If the user doesn't exist (404),
        // we tell them to register — we do NOT silently create an account.
        // Silently creating accounts during login was the root cause of the
        // ghost-user bug.
        try {
          ...
          if (isNotRegistered) {
            // Guide user to register — do NOT auto-register them
            toast.error(
              "No account found. Please register first.",
              { duration: 4000 }
            );
            switchMode("register");
            return; // stop processing — OTP stays valid for 10 min
          }

          throw loginErr; // re-throw all other errors (wrong OTP, locked, etc.)
        }
      }
      ...
      toast.success(
        mode === "register" ? "Account created! Welcome aboard." : "Login successful!"
      );
  ...
  // ── Mode switch (resets all OTP state) ───────────────────────────────────
  const switchMode = (m) => {
    setMode(m);
    setStep(1);
    setOtp("");
  ...
  return (
    ...
    >
      ...
      {/* Card */}
      <div className="relative z-10 w-full max-w-sm sm:max-w-md bg-white/96 backdrop-blur-lg
        rounded-2xl shadow-2xl p-6 sm:p-8 mt-24 lg:mt-0">
        ...
        {/* Booking redirect hint */}
        {redirectTo && (
          <div className="mb-4 px-3 py-2 rounded-lg bg-orange-50 border border-orange-200
        ...
        {/* ── Passenger Login / Register (OTP flow) ── */}
          ...
          <>
            {step === 1 ? (
              <button
                ...
                disabled={loading}
                className="w-full bg-[#0F3D2E] hover:bg-[#0c2e22] text-white py-2.5 rounded-xl
                  mb-3 flex justify-center items-center font-semibold transition-colors
                  disabled:opacity-60"
              >
        ...
        {mode === "staff" && (
          <form onSubmit={handleStaffLogin} className="space-y-3">
            <div className="relative">
              <input
                ...
                className="w-full border border-gray-300 rounded-xl px-4 py-2.5 pr-11 text-sm
                  outline-none focus:ring-2 focus:ring-[#0F3D2E]"
              />
              <button
                type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-orange-500 hover:bg-orange-600 text-white py-2.5 rounded-xl
                flex justify-center items-center font-semibold transition-colors disabled:opacity-60"
            >
