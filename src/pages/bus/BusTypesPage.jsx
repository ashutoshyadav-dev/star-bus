...
import {
  getAllBusTypes, createBusType, updateBusType, deactivateBusType,
} from "../../api/bus";
...
// =============================================================================
// SeatLayoutBuilder — visual builder for the seat_layout_template JSON.
// Generates the exact client layout: rows×cols, aisle gap, optional last-row-full.
// Admins click "Generate" → JSON is produced and sent to backend on save.
// =============================================================================
...
function generateTemplateJson(rows, cols, aisleCol, lastRowFull) {
  ...
  for (let r = 1; r <= rows; r++) {
    ...
    for (let c = 1; c <= cols; c++) {
      // Skip aisle column for all rows except last-row-full
  ...
  return JSON.stringify(
    { rows, cols, aisleColumn: aisleCol, lastRowFull, seats },
    null,
    2,
  );
}
...
function SeatLayoutBuilder({ value, onChange, onTotalSeatsChange }) {
  ...
  // Parse current JSON to build the mini preview
  let preview   = [];
  let pRows     = 0;
  let pCols     = 0;
  let pAisle    = 0;
  ...
  } catch (_) { /* value is empty or invalid — no preview */ }

  const handleGenerate = () => {
    const json = generateTemplateJson(rows, cols, aisleCol, lastRowFull);
    onChange(json);
    // Compute seat count and sync with the totalSeats field
    try {
      ...
      <label className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer
        ...
        <div>
          ...
          <p className="text-xs opacity-70">
            Last row spans all {cols} cols including aisle (like client layout seat 35–39)
          </p>
        </div>
      </label>
      ...
      {preview.length > 0 && (
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 overflow-x-auto">
          ...
          <div className="flex justify-end mb-1 pr-1">
            <span className="text-lg" title="Driver">🚌</span>
          </div>
...
export default function BusTypesPage() {
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      // Validate that JSON is present
      if (!form.seatLayoutTemplate || form.seatLayoutTemplate.trim() === "") {
      ...
      try { JSON.parse(form.seatLayoutTemplate); }
        ...
        setError("Seat layout template is not valid JSON. Use the Layout Builder.");
        setSaving(false);
        return;
      }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Define vehicle classes and seat layout templates
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {["Name", "Code", "Category", "Seats", "Multiplier",
                  "Features", "Status", "Actions"].map(h => (
                  <th key={h}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl
            ...
            <div className="p-6 space-y-5">
              ...
              {/* Row 2: Category + Seats + Multiplier */}
                ...
                {showBuilder ? (
                  ...
                  <div className={`px-4 py-3 rounded-xl border text-sm
                    ...
                    {form.seatLayoutTemplate
                      ...
                      : "No layout generated yet. Click 'Open Layout Builder' above."}
                  </div>
                )}
              </div>
